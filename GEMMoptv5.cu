...
int main() {
  ...
  std::vector<float> h_c_wmma_accum_float(static_cast<size_t>(M) * N); 
  std::vector<__half> h_c_wmma_half(static_cast<size_t>(M) * N);    
  ...
  cudaMemcpy(h_c_wmma_accum_float.data(), d_c_accum, bytes_c_accum, cudaMemcpyDeviceToHost);
...
__global__ void wmmaGemmKernel(const __half* __restrict__ a, const __half* __restrict__ b, float* __restrict__ c_float_accum, __half* d_c_ptr_unused, int M_param, int N_param, int K_param) {
  ...
  wmma::fragment<wmma::accumulator, WMMA_M, WMMA_N, WMMA_K, float> acc_frag_regions[num_m_region][num_n_region][2][2];
  ...
  for (int m_reg = 0; m_reg < num_m_region; ++m_reg) {
    for (int n_reg = 0; n_reg < num_n_region; ++n_reg) {
      wmma::fill_fragment(acc_frag_regions[m_reg][n_reg][0][0], 0.0f);
      wmma::fill_fragment(acc_frag_regions[m_reg][n_reg][0][1], 0.0f);
      wmma::fill_fragment(acc_frag_regions[m_reg][n_reg][1][0], 0.0f);
      wmma::fill_fragment(acc_frag_regions[m_reg][n_reg][1][1], 0.0f);
    }
  }
  ...
  for (int k_tile_idx = 0; k_tile_idx < num_k_tiles; ++k_tile_idx) {
    ...
    {
        const int a_tile_start_row_global = block_start_row_c;
        const int a_tile_start_col_global = k_tile_idx * TILE_K;
        constexpr int VECTOR_SIZE_A = 4;
        const int2 zero_vec_a = {0, 0};
        for (int i = 0; i < (TILE_M * TILE_K) / (BLOCK_THREADS * VECTOR_SIZE_A); ++i) {
            int linear_idx = threadIdx.x + i * BLOCK_THREADS;
            int load_s_a_row = (linear_idx * VECTOR_SIZE_A) / TILE_K;
            int load_s_a_col_start = (linear_idx * VECTOR_SIZE_A) % TILE_K;
            int load_global_a_row = a_tile_start_row_global + load_s_a_row;
            int load_global_a_col_start = a_tile_start_col_global + load_s_a_col_start;
            if (load_global_a_row < M_param && load_global_a_col_start < K_param) {
                *(reinterpret_cast<int2*>(&s_a[load_s_a_row][load_s_a_col_start])) =
                    *(reinterpret_cast<const int2*>(&a[load_global_a_row * K_param + load_global_a_col_start]));
            } else {
                *(reinterpret_cast<int2*>(&s_a[load_s_a_row][load_s_a_col_start])) = zero_vec_a;
            }
        }
    }

    {
        const int b_tile_start_row_global = k_tile_idx * TILE_K;
        const int b_tile_start_col_global = block_start_col_c;
        constexpr int VECTOR_SIZE_B = 4;
        const int2 zero_vec_b = {0, 0};
        for (int i = 0; i < (TILE_K * TILE_N) / (BLOCK_THREADS * VECTOR_SIZE_B); ++i) {
            int linear_idx = threadIdx.x + i * BLOCK_THREADS;
            int load_s_b_row = (linear_idx * VECTOR_SIZE_B) / TILE_N;
            int load_s_b_col_start = (linear_idx * VECTOR_SIZE_B) % TILE_N;
            int load_global_b_row = b_tile_start_row_global + load_s_b_row;
            int load_global_b_col_start = b_tile_start_col_global + load_s_b_col_start;
            if (load_global_b_row < K_param && load_global_b_col_start < N_param) {
                *(reinterpret_cast<int2*>(&s_b[load_s_b_row][load_s_b_col_start])) =
                    *(reinterpret_cast<const int2*>(&b[load_global_b_row * N_param + load_global_b_col_start]));
            } else {
                *(reinterpret_cast<int2*>(&s_b[load_s_b_row][load_s_b_col_start])) = zero_vec_b;
            }
        }
    }
    ...
    for (int m_reg_idx = 0; m_reg_idx < num_m_region; ++m_reg_idx) {
      for (int n_reg_idx = 0; n_reg_idx < num_n_region; ++n_reg_idx) {
        ...
        for (int k_wmma_idx = 0; k_wmma_idx < (TILE_K / WMMA_K); ++k_wmma_idx) {
          ...
          int s_a_load_base_row = warp_quadrant_m_offset + current_region_row_in_quadrant;
          int s_a_load_col = k_wmma_idx * WMMA_K;
          int s_b_load_base_col = warp_quadrant_n_offset + current_region_col_in_quadrant;
          int s_b_load_row = k_wmma_idx * WMMA_K;

          wmma::load_matrix_sync(a_frag[0], &s_a[s_a_load_base_row][s_a_load_col], TILE_K + 8);
          wmma::load_matrix_sync(a_frag[1], &s_a[s_a_load_base_row + WMMA_M][s_a_load_col], TILE_K + 8);
          wmma::load_matrix_sync(b_frag[0], &s_b[s_b_load_row][s_b_load_base_col], TILE_N + 8);
          wmma::load_matrix_sync(b_frag[1], &s_b[s_b_load_row][s_b_load_base_col + WMMA_N], TILE_N + 8);

          wmma::mma_sync(acc_frag_regions[m_reg_idx][n_reg_idx][0][0], a_frag[0], b_frag[0], acc_frag_regions[m_reg_idx][n_reg_idx][0][0]);
          wmma::mma_sync(acc_frag_regions[m_reg_idx][n_reg_idx][0][1], a_frag[0], b_frag[1], acc_frag_regions[m_reg_idx][n_reg_idx][0][1]);
          wmma::mma_sync(acc_frag_regions[m_reg_idx][n_reg_idx][1][0], a_frag[1], b_frag[0], acc_frag_regions[m_reg_idx][n_reg_idx][1][0]);
          wmma::mma_sync(acc_frag_regions[m_reg_idx][n_reg_idx][1][1], a_frag[1], b_frag[1], acc_frag_regions[m_reg_idx][n_reg_idx][1][1]);
        }
      }
    }
  ...
  for (int m_reg_idx = 0; m_reg_idx < num_m_region; ++m_reg_idx) {
    for (int n_reg_idx = 0; n_reg_idx < num_n_region; ++n_reg_idx) {
       ...
       if (c_store_row_0 < M_param && c_store_col_0 < N_param) {
         wmma::store_matrix_sync(&c_float_accum[c_store_row_0 * N_param + c_store_col_0], acc_frag_regions[m_reg_idx][n_reg_idx][0][0], N_param, wmma::mem_row_major);
       }
       ...
       if (c_store_row_1 < M_param && c_store_col_1 < N_param) {
         wmma::store_matrix_sync(&c_float_accum[c_store_row_1 * N_param + c_store_col_1], acc_frag_regions[m_reg_idx][n_reg_idx][0][1], N_param, wmma::mem_row_major);
       }
       ...
       if (c_store_row_2 < M_param && c_store_col_2 < N_param) {
         wmma::store_matrix_sync(&c_float_accum[c_store_row_2 * N_param + c_store_col_2], acc_frag_regions[m_reg_idx][n_reg_idx][1][0], N_param, wmma::mem_row_major);
       }
       ...
       if (c_store_row_3 < M_param && c_store_col_3 < N_param) {
         wmma::store_matrix_sync(&c_float_accum[c_store_row_3 * N_param + c_store_col_3], acc_frag_regions[m_reg_idx][n_reg_idx][1][1], N_param, wmma::mem_row_major);
       }
    }
  }
}
